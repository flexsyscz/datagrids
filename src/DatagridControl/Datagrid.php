...
abstract class Datagrid extends Control
{
	...
	public const    OFFSET_COLUMNS = 'columns',
					OFFSET_SELECTED_ITEMS = 'selectedItems';
	...
	public function getAdjustableColumns(): array
	{
		...
		if($session->offsetExists(self::OFFSET_COLUMNS)) {
			$_columns = $session->offsetGet(self::OFFSET_COLUMNS);
	...
	protected function createComponentAdjustColumnsForm(): Form
	{
		...
		$form->addCheckboxList(self::OFFSET_COLUMNS, $this->translate('!datagrid.adjust.form.columns.label'))
			->setRequired(false)
			->setItems($items);
		...
		$form->onSuccess[] = function(Form $form) {
			try {
				...
				$this->getSession()->offsetSet(self::OFFSET_COLUMNS, $columns);
			} catch (InvalidArgumentException $e) {
				$form->addError($e->getMessage());
				return;
			}
	...
	public function handleToggleItem(int $itemId): void
	{
		...
		$items = [];
		$session = $this->getSession();
		if ($session->offsetExists(self::OFFSET_SELECTED_ITEMS)) {
			$items = $session->offsetGet(self::OFFSET_SELECTED_ITEMS);
			if (!is_array($items)) {
				$items = [];
			}
		}
		...
		if ($item instanceof IEntity) {
			...
			$session->offsetSet(self::OFFSET_SELECTED_ITEMS, $items);
	...
	public function handleToggleAllItems(): void
	{
		...
		$session->offsetSet(self::OFFSET_SELECTED_ITEMS, $items);
		$payload = [self::OFFSET_SELECTED_ITEMS => $items];
	...
	public function flushSelectedItems(): void
	{
		$session = $this->getSession();
		if ($session->offsetExists(self::OFFSET_SELECTED_ITEMS)) {
			$session->offsetUnset(self::OFFSET_SELECTED_ITEMS);
		}
	}
}
